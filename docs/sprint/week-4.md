...
**起步校准（8 分钟，零基础必做）：** 在[频域工具章](../guide/03A-交流相量与频域工具.md)依次查“复数的模与相角”“相量为什么把微分变成乘 $j\omega$”“阻抗”“分贝”和“一阶 Bode 规则”。在纸上写出 $|1+j|=\sqrt2$、$Z_C=1/(j\omega C)$、$20\log_{10}(1/\sqrt2)=-3.01\ \mathrm{dB}$；三式都能解释后再继续。本工具章的拉普拉斯证明、复功率和高阶例题留到第二遍。

**定位资料（非整节通读）：** 起步校准通过后，只查[5.1 一阶低通方程](../guide/06-反馈与频率响应.md#51-一阶低通输出取在电容上)、5.3 $-3\ \mathrm{dB}$/斜率块、[第 6 节时间—频率对照图](../guide/06-反馈与频率响应.md#6-p0同一个-rc-的时间域频率域联系)和[10.2 稳定裕度图](../guide/06-反馈与频率响应.md#102-增益交越相位裕度与增益裕度)；Miller 与补偿推导不读。
