...
前置知识是[第 1 章](01-最小电路基础.md)的 KCL/KVL、戴维南等效和局部线性化，以及[第 2 章](02-二极管与半导体基础.md)的 PN 结偏置、指数方程与状态假设算法。默认温度 \(T=300\ \mathrm K\)，因此热电压
...
| 路线 | 阅读范围 | 一个月备考动作 |
|---|---|---|
| 冲刺轨 | 所有 P0、四个完整直流例题、十二个状态案例、30 秒回答、T-01～T-10 | 约 5 小时，至少分两次；每次计算都写工作区检查 |
| 深入轨 | 90 秒回答、Early 效应、沟道长度调制、体效应、反向有源与全部追问 | P0 过关后补，用于回答“为什么”和“何时失效” |
...
只列题目，不给完整答案。每道计算题都应写“假设 → 模型 → 变量与参考方向 → 基本方程 → 求解 → 量纲 → 极限 → 失效条件”，每道趋势题都要写因果链并说明保持不变的量。更多同类题见[练习题总表](../exercises/练习题.md)。

### T-01　端子识别、参考方向与载流子

画出一个 NPN 和一个增强型 NMOS。对 NPN 标出 E/B/C、\(I_B,I_C\) 流入、\(I_E\) 流出、\(V_{BE}=V_B-V_E,V_{CE}=V_C-V_E\)，并画出正向有源时电子从发射极经薄基区到集电极的路径；对 NMOS 标出 G/D/S/B、体接源、\(I_D\) 入漏出源、\(V_{GS},V_{DS}\)，并画出正栅压下的反型沟道。解释传统电流与电子运动方向。假设普通 NPN、增强型 NMOS、低频示意、\(V_{DS}\ge0\)；本题不使用 \(\beta,V_{TH}\) 或温度数值。[详细解答](../exercises/详细解答.md#t-01)｜[题库同类题](../exercises/练习题.md)

### T-02　由端电压判断 BJT 区域

同一普通硅 NPN 依次测得三组端电压（单位 V）：A. \((V_E,V_B,V_C)=(0.40,1.10,4.20)\)；B. \((0,0.10,5.00)\)；C. \((0,0.80,0.20)\)。对每组计算 \(V_{BE},V_{BC},V_{CE}\)，依据两个结的偏置判断区域。采用分段判据：结在约 \(0.70\ \mathrm V\) 正偏，饱和模型为 \(V_{BE,\mathrm{sat}}=0.80\ \mathrm V,V_{CE,\mathrm{sat}}=0.20\ \mathrm V\)；假设 \(T=300\ \mathrm K\)、未击穿，区域判断不使用 \(\beta\)。[详细解答](../exercises/详细解答.md#t-02)｜[题库同类题](../exercises/练习题.md)

### T-03　固定偏置的 Q 点与区域检查

一个 NPN 的发射极接地，\(R_B=330\ \mathrm{k\Omega}\) 从 \(V_{CC}=9.00\ \mathrm V\) 接到基极，\(R_C=1.80\ \mathrm{k\Omega}\) 从 \(V_{CC}\) 接到集电极。求 \(I_B,I_C,I_E,V_B,V_C,V_{CE},V_{BC}\) 和 Q 点，画直流负载线并检查区域。假设 \(T=300\ \mathrm K\)、正向有源候选模型 \(V_{BE}=0.70\ \mathrm V,\beta=80\)，饱和候选模型 \(V_{BE,\mathrm{sat}}=0.80\ \mathrm V,V_{CE,\mathrm{sat}}=0.20\ \mathrm V\)，忽略 Early 效应和自热。[详细解答](../exercises/详细解答.md#t-03)｜[题库同类题](../exercises/练习题.md)

### T-04　BJT 的 \(g_m,r_\pi,r_e\) 与线性化尺度

某 NPN 已偏置在正向有源区，\(I_{CQ}=1.50\ \mathrm{mA}\)，局部 \(\beta=120\)，叠加峰值 \(|\tilde v_{be}|=4.00\ \mathrm{mV}\)。求 \(g_m,r_\pi,r_e\)（分别给精确 \(\alpha/g_m\) 与 \(1/g_m\) 近似），再用二阶/一阶项尺度估计线性化误差数量级。假设 \(T=300\ \mathrm K,V_T=25.9\ \mathrm{mV}\)，忽略 Early 效应、结电容和温度摆动。[详细解答](../exercises/详细解答.md#t-04)｜[题库同类题](../exercises/练习题.md)

### T-05　【只判断趋势】发射极反馈与参数变化

对图 3-8 的分压—发射极电阻偏置，每次只改变一个量并判断 \(I_C,V_E,V_{CE}\) 的首要变化趋势及反馈链：A. \(\beta\) 从 50 增到 150；B. \(R_E\) 增大 20%；C. 温度升高使所选恒压降参数 \(V_{BE}\) 降低，而其他模型参数暂固定；D. \(R_{\mathrm{TH}}\) 降低而 \(V_{\mathrm{TH}}\) 不变。只判断趋势，不求新数值。沿用图 3-8 其余参数、\(T\) 以 300 K 为起点、候选正向有源 \(V_{BE}=0.70\ \mathrm V\)，忽略 Early 效应和自热；每问最后检查趋势是否可能把器件推向区域边界。[详细解答](../exercises/详细解答.md#t-05)｜[题库同类题](../exercises/练习题.md)

### T-06　MOS 的 \(V_{OV}\)、区域与电流

一个体接源的增强型 NMOS 有 \(V_{TH}=0.80\ \mathrm V,k_n=0.80\ \mathrm{mA/V^2}\)，固定 \(V_{GS}=2.60\ \mathrm V\)。分别在 A. \(V_{DS}=0.70\ \mathrm V\) 与 B. \(V_{DS}=3.00\ \mathrm V\) 时求 \(V_{OV}\)、判断区域并计算 \(I_D\)，再验证在 \(V_{DS}=V_{OV}\) 处两式连续。假设 \(T=300\ \mathrm K\)、长沟道强反型、\(V_{DS}\ge0\)、忽略 \(\lambda\)、体效应、自热和亚阈值电流。[详细解答](../exercises/详细解答.md#t-06)｜[题库同类题](../exercises/练习题.md)

### T-07　栅分压—源电阻偏置的物理解

\(V_{DD}=9.00\ \mathrm V\)，\(R_1=2.00\ \mathrm{M\Omega}\) 从电源接栅，\(R_2=1.00\ \mathrm{M\Omega}\) 从栅接地，\(R_D=1.50\ \mathrm{k\Omega}\)，\(R_S=0.750\ \mathrm{k\Omega}\)。求全部代数候选根，按 \(V_{OV}\) 和区域条件筛出物理解，再给 \(I_D,V_G,V_S,V_D,V_{GS},V_{DS}\) 与 Q 点。假设 \(T=300\ \mathrm K\)、体接源、理想 \(I_G=0\)、\(V_{TH}=1.00\ \mathrm V,k_n=1.20\ \mathrm{mA/V^2}\)、长沟道饱和候选、\(\lambda=0\)。[详细解答](../exercises/详细解答.md#t-07)｜[题库同类题](../exercises/练习题.md)

### T-08　【只判断趋势】MOS 偏置、\(g_m\) 与 \(r_o\)

对图 3-16，每次只改变一个量，判断 \(I_D,V_S,V_{OV},g_m\) 的首要趋势：A. \(V_{TH}\) 增大；B. \(R_S\) 增大；C. \(R_D\) 增大且器件尚未跨区；D. 保持 \(I_{DQ},V_{DSQ}\) 不变而把 \(\lambda\) 加倍，判断 \(r_o\)。只判断趋势，不求数值。沿用图 3-16 的 300 K 长沟道模型、体接源、理想 \(I_G=0\)；A～C 先忽略 \(\lambda\)，D 使用局部结果 \(r_o=(1+\lambda V_{DSQ})/(\lambda I_{DQ})\)，每问检查饱和边界。[详细解答](../exercises/详细解答.md#t-08)｜[题库同类题](../exercises/练习题.md)

### T-09　【只判断趋势】模型有效性压力测试

从本章模型出发，每次只作一种变化，判断“误差趋增还是趋减”，并指出首先需要补入的效应：A. BJT 的 \(|\tilde v_{be}|\) 从 \(1\ \mathrm{mV}\) 增到 \(20\ \mathrm{mV}\)；B. NMOS 的 \(|\tilde v_{gs}|/V_{OVQ}\) 从 0.01 增到 0.5；C. 信号频率提高到栅电容或结电容阻抗与电路电阻同量级；D. 用现代短沟道 NMOS 代替长沟道器件；E. BJT 的 \(V_{CE}\) 被负载拉近饱和。只判断趋势，不求数值。起点为 \(T=300\ \mathrm K\)、BJT 正向有源且忽略 Early 效应、NMOS 体接源且在长沟道饱和区；不指定 \(\beta,V_{TH}\)，因为题目只检查模型边界。[详细解答](../exercises/详细解答.md#t-09)｜[题库同类题](../exercises/练习题.md)

### T-10　【只判断趋势】BJT/MOS 端口与跨导比较

比较偏置电流同为 \(1.00\ \mathrm{mA}\) 的 NPN 与长沟道强反型 NMOS：A. 由 \(g_m/I\) 判断当 NMOS 的 \(V_{OV}\) 从 \(0.40\ \mathrm V\) 降到 \(0.20\ \mathrm V\) 时，两者跨导差距怎样变化；B. 输入频率提高时，两者输入端“近似开路”的说法各怎样变化；C. 若要求偏置对器件参数离散不敏感，两者各应采用什么类型的局部直流反馈。只判断趋势与机制，不计算最终数值。假设 \(T=300\ \mathrm K,V_T=25.9\ \mathrm{mV}\)，NPN 在正向有源且局部 \(\beta=100\)，NMOS 体接源、饱和、\(\lambda=0\)，比较式限定为 \(g_{m,\mathrm{BJT}}=I_C/V_T\) 与 \(g_{m,\mathrm{MOS}}=2I_D/V_{OV}\)。[详细解答](../exercises/详细解答.md#t-10)｜[题库同类题](../exercises/练习题.md)

完成后，不要只核对数值。把错因归到“端口方向、器件物理、模型选择、工作区、方程、物理解筛选、小信号尺度、非理想边界”之一，再回到对应 P0 小节出声复述 30 秒回答。
